-- Baseline schema for the LOCAL Supabase stack only (`supabase start` / `supabase db reset`).
-- This is a hand-kept copy of /supabase-schema.sql, the actual source of truth for the live
-- project (same "kept in sync by hand" convention as database.types.ts / calculatePMC.ts — see
-- CLAUDE.md). The live project itself is NOT managed through this migrations directory: schema
-- changes to the live project still go through mcp__supabase__apply_migration per CLAUDE.md's
