...
src/
  ...
  hooks/         — useAuth, useIsMobile, useAICoachData (AI Coach fetch/derived-metric logic), useGoals (Season Goals CRUD), usePWAUpdate (wraps `virtual:pwa-register/react`'s `useRegisterSW`)
  ...
  components/
    ...
    library/     — LibraryPage
    PWAUpdateToast.tsx — floating "update available" banner, shown when a new service worker build is waiting (see PWA install support above)
    LogWorkoutModal.tsx
...
- PWA install support: `vite-plugin-pwa` (`generateSW` mode) generates the service worker at build time from config in `vite.config.ts`; `public/manifest.webmanifest` is hand-authored (not plugin-generated) so its contents stay one plain, diffable file. Precaches only the built app shell (JS/CSS/HTML/icons) — a `NetworkOnly` Workbox rule explicitly excludes all `*.supabase.co` traffic (REST/Auth/Realtime/Storage/Edge Functions) from ever being cached or served stale; Realtime's WebSocket upgrade isn't interceptable by a service worker regardless. `registerType: 'prompt'` (not `autoUpdate`) — an update installs in the background and waits for the user to act via `PWAUpdateToast` (`src/components/PWAUpdateToast.tsx`, backed by `src/hooks/usePWAUpdate.ts` wrapping `useRegisterSW` from `virtual:pwa-register/react`), rather than silently reloading and discarding an open LogWorkoutModal form. Icons (`pwa-192x192.png`/`pwa-512x512.png`/`pwa-maskable-512x512.png`/`apple-touch-icon.png`/`favicon-32x32.png`) are generated from `public/vexr-new.png` padded to a square canvas. This is installability + a graceful offline app-shell fallback only — not the separate, larger "offline mode" roadmap item (no real offline data sync).
...
- 35 test files / 415 tests, each co-located in `__tests__/` beside the file under test
